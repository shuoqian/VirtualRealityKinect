				   ...
				   )
{
	...
	{
		/*x=x_icp+y_icp*aa-z_icp*cc+ttaa;
		y=-1*x_icp*aa+y_icp+z_icp*bb+ttbb;
		z=x_icp*cc-y_icp*bb+z_icp+ttcc;	*/
...
__kernel void addone(__global unsigned short* mat1,
					 __global unsigned short* mat2,
					 __global unsigned int* point_num,
					 __global float* point,
					 ...
					 )
{
	...
	mat2[idx+idy*gsizex]=mat1[idx+idy*gsizex]*60;
	valid[idx+idy*gsizex]=0;
	bool valid_this=false;
//////////////////////////////////////////////////////////////////////////

//	uint half_area=5;
	float value=doub_depth[idx+idy*gsizex];
...
//		*point_num++;
	...
	{
		//是屏幕中间部分的点
		//if(valid_center==true)//(u<639-*half_area&v<479-*half_area&u>*half_area+1&v>*half_area+1)
		//{
			/*x_left=point[(size_t)(u-*half_area)*3+idy*gsizex*3];
			x_right=point[(size_t)(u+*half_area)*3+idy*gsizex*3];
			x_up=point[idx*3+(size_t)(v-*half_area)*gsizex*3];
			x_down=point[idx*3+(size_t)(v+*half_area)*gsizex*3];
			
			y_left=point[(size_t)(u-*half_area)*3+idy*gsizex*3+1];
			y_right=point[(size_t)(u+*half_area)*3+idy*gsizex*3+1];
			y_up=point[idx*3+(size_t)(v-*half_area)*gsizex*3+1];
			y_down=point[idx*3+(size_t)(v+*half_area)*gsizex*3+1];
			
			z_left=point[(size_t)(u-*half_area)*3+idy*gsizex*3+2];
			z_right=point[(size_t)(u+*half_area)*3+idy*gsizex*3+2];
			z_up=point[idx*3+(size_t)(v-*half_area)*gsizex*3+2];
			z_down=point[idx*3+(size_t)(v+*half_area)*gsizex*3+2];*/
			
			
			
			z_right=area[idx*4+idy*gsizex*4];
			z_left=area[idx*4+idy*gsizex*4+1];
			z_up=area[idx*4+idy*gsizex*4+2];
			z_down=area[idx*4+idy*gsizex*4+3];
			
			x_left=z_left*(u-(int)*half_area-320)/525;
			x_right=z_right*(u+(int)*half_area-320)/525;
			x_up=z_up*(u-320)/525;
			x_down=z_down*(u-320)/525;
			
			y_left=z_left*(v-240)/525;
			y_right=z_right*(v-240)/525;
			y_up=z_up*(v-(int)*half_area-240)/525;
			y_down=z_down*(v+(int)*half_area-240)/525;
			
			
			if(z_left>0) valid_left=true;
			if(z_right>0) valid_right=true;
			if(z_up>0) valid_up=true;
			if(z_down>0) valid_down=true;
			
			uint k=1;
			/*for(uint i=0;i<*half_area*2+1;i++)
				for(uint j=0;j<*half_area*2+1;j++)
				{
					k=k*point[(size_t)((u-(int)*half_area+i)*3+(v-(int)*half_area+j)*gsizex*3+2)];
		...
		if(valid_center&z_left>0&z_right>0&z_up>0&z_down>0)//&(valid_area==true)&(fabs(z-z_left)<70)&(fabs(z-z_right)<70)&(fabs(z-z_up)<70)&(fabs(z-z_down)<70))
		{
			valid_this=true;
		}
		if(valid_this==true)
...
//////////////////////////////////////////////////////////////////预处理结束///////////////////////////////////////////////////////////////////////////
	...
	//norm=cross((float4)(x_right-x_left,y_right-y_left,z_right-z_left,0),(float4)(x_up-x_down,y_up-y_down,z_up-z_down,0));
	if(valid[idx+idy*gsizex]>0&valid_last[idx+idy*gsizex]>0)
	{
		
		//normal=fast_normalize(norm);
		normalization[idx*3+idy*gsizex*3]=normal.x;
		normalization[idx*3+idy*gsizex*3+1]=normal.y;
		normalization[idx*3+idy*gsizex*3+2]=normal.z;
	}
	else
	{
		//normal=(float4)(0);
		normalization[idx*3+idy*gsizex*3]=0;
		normalization[idx*3+idy*gsizex*3+1]=0;
		normalization[idx*3+idy*gsizex*3+2]=0;
	}
	
	//barrier(CLK_GLOBAL_MEM_FENCE);
	normalization[idx*3+idy*gsizex*3+2]=-1*normalization[idx*3+idy*gsizex*3+2];//最后要删掉这句
	normalization[idx*3+idy*gsizex*3+1]=-1*normalization[idx*3+idy*gsizex*3+1];//最后要删掉这句
	normalization[idx*3+idy*gsizex*3]=-1*normalization[idx*3+idy*gsizex*3];//最后要删掉这句
	

	float4 distance=(float4)(0);
	if(valid[idx+idy*gsizex]>0&valid_last[idx+idy*gsizex]>0)
	{
		distance.x=point[idx*3+idy*gsizex*3]-point_last[idx*3+idy*gsizex*3];
		distance.y=point[idx*3+idy*gsizex*3+1]-point_last[idx*3+idy*gsizex*3+1];
		distance.z=point[idx*3+idy*gsizex*3+2]-point_last[idx*3+idy*gsizex*3+2];
		if(fabs(distance.z)<0&fabs(dot(fast_normalize(distance),normal))>1)
		{
			//energy[idx+idy*gsizex]=fabs(dot(distance,normal))/100;
			//valid[idx+idy*gsizex]=0;
		}
		else
		{
			//energy[idx+idy*gsizex]=0;
			valid[idx+idy*gsizex]=0;
		}
	}
	else
